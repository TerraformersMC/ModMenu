...
public class ModMenuButtonWidget extends ButtonWidget {
	...
	public ModMenuButtonWidget(int x, int y, int width, int height, Text text, Screen screen, Screen altScreen) {
		super(x, y, width, height, text, button -> {
			if (Screen.hasAltDown()) {
				MinecraftClient.getInstance().setScreen(altScreen);
			} else {
				MinecraftClient.getInstance().setScreen(new ModsScreen(screen));
			}
